...
**Status: wired, as a demo cache-first path.** `app.services.chortlechat` now loads `backend/data/preset_qa.json` into a cache keyed by exact question text (entries with `use_cache: true` only) and, on a hit, answers straight from `baseline_answer` — no retrieval call, no confidence-threshold gate, no Baseline generation call. These 14 questions are curated and corpus-verified ahead of time, so for demo purposes they always answer instantly and consistently, unaffected by live vector-store or embedding-provider issues. `confidence` stays `None` on these responses (same meaning it already carries for "nothing was retrieved," which is literally true here). Banter is untouched: it still always makes one live call to restyle whichever Baseline answer is in play. See `backend/tests/test_chortlechat.py`'s "Preset chip-question cache" section for the covering tests.
