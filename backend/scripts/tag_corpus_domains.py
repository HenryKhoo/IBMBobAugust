"""One-time reclassification: tag backend/data/talkback_corpus.json with `domain`.
...
line (see fetch_talkback_corpus.py's `_flatten`). This maps each of the
...
This only rewrites backend/data/talkback_corpus.json — it does not touch
Zilliz. Re-run backend/scripts/ingest_talkback_corpus.py afterward (with
...
app.services.talkback._retrieval_expr's fallback).

If a future corpus refresh (re-running fetch_talkback_corpus.py) ever
...
# fetch_talkback_corpus.py, classified by hand against the five domains a
...
def main() -> None:
    if not CORPUS_PATH.exists():
        raise SystemExit(
            f"{CORPUS_PATH} not found. Run backend/scripts/fetch_talkback_corpus.py first."
        )
    ...
    print(
        f"\nWrote {CORPUS_PATH}. Re-run backend/scripts/ingest_talkback_corpus.py "
