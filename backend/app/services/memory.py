...
`app.services.talkback`). This module adds two layers on top so a natural
...
The grounding discipline in `app.services.talkback` is unchanged by either
...
retrieves. A wrong or stale memory can make Talkback answer the wrong
question; it can never make Talkback state an ungrounded claim.
...
def persist_grounded_exchange(
    ...
    Only ever called for a grounded exchange (see `app.services.talkback`)
...
def recall_relevant_history(store, session_id: str, question: str, *, k: int = _HISTORY_RECALL_K):
    ...
    Used by `app.services.talkback.ask_talkback` only when the in-process
