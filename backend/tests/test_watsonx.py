...
def test_instruct_model_is_watsonx_only_regardless_of_a_configured_gemini_key(monkeypatch):
    """Generation never involves Gemini — GEMINI_API_KEY only affects
    embeddings (see the tests below). A Gemini key being set must not
    change what get_instruct_model returns."""
    _stub_chat_watsonx(monkeypatch)
    monkeypatch.setattr(watsonx.settings, "WATSONX_INSTRUCT_MODEL_ID", "granite-primary")
    monkeypatch.setattr(watsonx.settings, "WATSONX_INSTRUCT_MODEL_FALLBACK_ID", "llama-fallback")
